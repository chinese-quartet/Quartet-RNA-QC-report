...
#' @export
exp2qcdt <- function(exp_table_file, count_table_file, phenotype_file, result_dir = NULL) {
  # Global variable
  ...
  if (is.null(result_dir)) {
    path <- getwd()
    result_dir <- file.path(path, "output")
  ...
  if (length(unique(dt_meta$sample)) < 2 & all(table(dt_meta[, "sample"]) < 2)) {
    ...
    rna_report_result <- make_performance_plot(dt_fpkm, dt_fpkm_log, dt_counts, dt_meta, result_dir, abs_cor_median, pt_abs_median_cor, ref_data)
  }
  return(rna_report_result)
}
